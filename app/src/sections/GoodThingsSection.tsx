...
export function GoodThingsSection() {
  ...
  const productA = products[5]; // Ceramic Mug
  const productB = products[6]; // Linen Napkin Set

  return (
    <section
      ref={sectionRef}
      className="section-pinned bg-[#F6F6F2] relative overflow-hidden z-30"
    >
      {/* Gradient Overlay */}
      <div className="absolute inset-0 gradient-mint pointer-events-none" />

      {/* Decorative Elements */}
      <motion.div
        style={{ opacity: contentOpacity }}
        className="absolute right-[10vw] top-[12vh] w-5 h-5 rounded-full bg-[#8fe8b8]/40"
      />
      <motion.div
        style={{ opacity: contentOpacity }}
        className="absolute left-[30vw] bottom-[20vh] decorative-ring w-24 h-24 opacity-20"
      />
        ...
        <div className="w-full px-6 lg:px-[6vw]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            <motion.div style={{ opacity: contentOpacity }}>
              ...
              <motion.h2
                style={{ x: headlineX }}
                className="heading-display text-[clamp(40px,6vw,96px)] text-[#111]"
              >
                GOOD
                <br />
                THINGS
              ...
              >
                Curated with care. Packed with love. Delivered with a smile.
              </motion.p>
